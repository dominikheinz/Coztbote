...
def packet_station_behavior(robot):
    ...
    try:
        cube = search_for_cube(robot, 10)
        print("Cube found")
        RobotStatusController.perceived_cubes.append(cube)
        print("Cube observed: " + cube.descriptive_name)
        robot.pickup_object(RobotStatusController.perceived_cubes[0], False, False, 10).wait_for_completed()
        # robot.drive_straight(distance_mm(-100), Speed(20), True, False, 3).wait_for_completed()
        robot.turn_in_place(degrees(220), False, 1).wait_for_completed()
