...
class SignHandler:
    ...
    def react_to_signs(self, sign_count):
        ...
        if (sign_count % 2) == 1:
            ...
            SignHandler.trigger_sign_detection_cooldown()
            self.drive_controller.stop_autonomous_behaviour()
            turn_action = self.robot.turn_in_place(degrees(180), degrees(180))
            turn_duration = (180 / Settings.cozmo_turn_speed_degrees_per_second) * 1000
            TimingUtils.run_function_after_if_action_finished(turn_duration, turn_action, self.drive_controller.continue_autonomous_behaviour)
